...
// Global variables for the service isolate
...
Future<void> initializeService() async {
  ...
  // নোটিফিকেশন লিসেনার সেটআপ
  AwesomeNotifications().setListeners(
    onActionReceivedMethod: BackgroundServiceManager.onActionReceivedMethod,
  );
...
void onStart(ServiceInstance service) async {
  ...
  // command listener
  service.on(COMMAND_STOP).listen((_) {
    debugPrint("Stop signal received in Service!");
    _stopAllTracking(service);
  });
...
void _startTimer(ServiceInstance service) {
  ...
  _timer = Timer.periodic(const Duration(seconds: 1), (_) {
    if (!_isTrackingPaused) {
      _timeElapsedSeconds++;
      if (_timeElapsedSeconds % 3 == 0) _sendTrackingData(service);
    }
  });
}
...
Future<void> _startNewTracking(ServiceInstance service) async {
  if (_isTrackingRunning && _isTrackingPaused) return _resumeTracking(service);
...
void _stopAllTracking(ServiceInstance service) async {
...
// Data Communication & Notification Update
void _sendTrackingData(ServiceInstance service) {
  ...
  AwesomeNotifications().createNotification(
    content: NotificationContent(
      ...
      title: _isTrackingPaused ? 'FitTrac: Paused' : 'FitTrac: Running $dist km',
      body: 'Time: $timeStr | Steps: $_totalSteps',
...
// Persistence
Future<void> _saveFinalTrackingData() async {
